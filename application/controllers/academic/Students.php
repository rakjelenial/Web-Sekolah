...
class Students extends Admin_Controller {
	...
	private function validation( $id = 0) {
		...
		$val->set_rules('nik', 'NIK', 'trim|callback_nik_exists[' . $id . ']');
		$val->set_rules('nisn', 'NISN', 'trim|callback_nisn_exists[' . $id . ']');
		...
		if ($this->input->is_ajax_request()) {
			...
			foreach($query->result() as $row) {
				...
				if ( $user_id == 0 ) {
					...
					if ( $insert_id ) $counter++;
				}
			}
			$this->vars['status'] = $counter > 0 ? 'success' : 'warning';
			$this->vars['message'] = $counter > 0 ? 'Akun sudah diaktifkan' : 'Akun sudah aktif sebelumnya';
			$this->output
				->set_content_type('application/json', 'utf-8')
				->set_output(json_encode($this->vars, JSON_HEX_APOS | JSON_HEX_QUOT))
				->_display();
